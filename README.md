...
> 버전 0.3은 선택형 Agent Skills 설치와 로컬 Codex 플러그인 빌드를 지원합니다. 자동화 레시피는 비활성 상태로만 복사합니다. 프로바이더 항목은 구성 선택지일 뿐 실제 커넥터가 아니며, 이 설치 도구는 어떤 레시피도 예약하거나 활성화하지 않습니다.
...
| `code-explainer` | 처음 보는 코드의 호출·상태·실패 경로를 이해하기 어려운 문제 | 실제 구현 근거와 HTML 설명 형식 |
...
- `$my-agent-stack:code-explainer 이 함수만 짧게 설명해줘` — 파일 변경 없이 필요한 범위만 설명합니다.
- `$my-agent-stack:code-explainer 이 기능의 호출 흐름을 HTML 설명 문서로 작성해줘` — [기본 형식](skills/code-explainer/assets/explainer.html)의 목차·도식·상세 접기를 사용합니다.
